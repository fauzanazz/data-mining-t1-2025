"""Config-based runner for FP Mining Pipeline.

This module provides functionality to run FP mining pipelines
...
from fp_mining.core.pipeline import FPMiningPipeline, PipelineResult
from fp_mining.algorithms import AprioriAlgorithm, FPGrowthAlgorithm
from fp_mining.loaders import (
    CSVLoader,
...
class FPConfigRunner:
    """Run FP mining pipeline from YAML configuration.

    Example:
        >>> runner = FPConfigRunner()
        >>> result = runner.run("config/fp_mining.yaml")
    """
    ...
    TRANSFORMER_TYPES = {
        "retail": RetailTransactionTransformer,
        "basket": BasketTransformer,
    }

    # Registry of available algorithms
    ALGORITHM_TYPES = {
        "apriori": AprioriAlgorithm,
        "fpgrowth": FPGrowthAlgorithm,
    }

    # Registry of available evaluators
    EVALUATOR_TYPES = {
        "coverage": CoverageEvaluator,
        "quality": RuleQualityEvaluator,
    ...
    def _create_algorithm(self, config: AlgorithmConfig) -> Any:
        ...
        if "max_len" in config.params and config.params["max_len"] is not None:
            params["max_len"] = config.params["max_len"]
    ...
    def _build_pipeline(self, config: PipelineConfig) -> FPMiningPipeline:
        """Build pipeline from configuration."""
        pipeline = FPMiningPipeline(verbose=config.verbose)
    ...
    def _save_json(
        ...
        output_file = output_dir / f"fp_results_{timestamp}.json"
        ...
        for dataset_name, algos in result.results.items():
            ...
            for algo_name, algo_data in algos.items():
                ...
                output_data["results"][dataset_name][algo_name] = {
                    "num_itemsets": len(algo_result.itemsets),
    ...
    def _print_summary(
        ...
        print("\n" + "=" * 60)
        print(f"FP MINING RESULTS: {config.name}")
        print("=" * 60)
...
def main() -> int:
    """CLI entry point for config-based FP mining."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Run FP Mining Pipeline from YAML configuration",
